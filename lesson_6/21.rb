...
loop do
  ...
  loop do
    ...
    if player_turn == 'h'
      player_cards << deck.pop
      prompt 'You hit!'
      prompt "Your cards are now: #{player_cards}, totaling #{total(player_cards)}"
    end

    break if player_turn.start_with?('s') || busted?(player_cards)
  end
  ...
  if busted?(dealer_cards)
    prompt "Dealer's total is now: #{total(dealers_cards)}"
    display_result(dealer_cards, player_cards)
    play_again? ? next : break
  else
